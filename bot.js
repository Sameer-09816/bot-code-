...
const axiosRetry = require('axios-retry').default; // NEW: Import axios-retry
...
// NEW: Create a dedicated axios instance for API calls
const apiClient = axios.create({
    headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36' },
    timeout: 30000 // MODIFIED: Increased timeout to 30 seconds
});

// NEW: Configure automatic retries for network errors
...
if (!TELEGRAM_BOT_TOKEN) {
    ...
    log('error', "Please provide the TELEGRAM_BOT_TOKEN environment variable.");
    process.exit(1);
}

const isProduction = !!APP_URL;
const bot = new TelegramBot(TELEGRAM_BOT_TOKEN, { polling: !isProduction });

// --- Webhook Setup (for Pella.app) ---
if (isProduction) {
    if (!APP_URL) {
        log('error', "!!! APP_URL NOT FOUND !!!");
        log('error', "Please provide the APP_URL environment variable for production.");
        process.exit(1);
    }
    ...
    bot.setWebHook(fullWebhookUrl).then(() => {
        log('info', `Webhook set to ${fullWebhookUrl}`);
    }).catch(err => {
        log('error', `Failed to set webhook: ${err.message}`);
    });

    const app = express();
    app.use(express.json());
    app.get('/', (req, res) => res.send('Instagram Downloader Bot is alive!'));
    app.post(webhookPath, (req, res) => {
        bot.processUpdate(req.body);
        res.sendStatus(200);
    });
    app.listen(PORT, () => {
        log('info', `Server is running on port ${PORT} and listening for webhooks.`);
    });
} else {
    log('info', 'Running in development mode with polling.');
    bot.on('polling_error', (error) => log('error', `Polling error: ${error.code} - ${error.message}`));
}
...
async function processSingleLink(chatId, url) {
    try {
        // MODIFIED: Use the new apiClient with retry logic
        ...
        for (const item of data.media) {
            ...
            try {
                // MODIFIED: Use the standard axios for HEAD request, as it's less critical
        ...
        if (mediaStreams.length > 1) {
            ...
            // FIXED: Added fileOptions with contentType to remove deprecation warning
            const fileOptions = {
        ...
        // MODIFIED: More specific error reason
...
// --- Message Handlers (Minor change to regex to avoid self-triggering) ---

bot.onText(/https?:\/\/(?:www\.)?instagram\.com/, async (msg) => {
    if (msg.text.startsWith('/')) return;
    // MODIFIED: Added check to prevent the bot from processing links it sends itself in summary messages
...
bot.on('document', async (msg) => {
    ...
    try {
        ...
        if (links.length === 0) {
            await bot.editMessageText("❌ No valid Instagram links found in the file.", {
        ...
        for (let i = 0; i < totalLinks; i += CONCURRENCY_LIMIT) {
            ...
            await bot.editMessageText(`⏳ Processing batch... (${processedCount}/${totalLinks} complete)`, {
                chat_id: chatId, message_id: statusMessage.message_id
            });
        ...
        summary += `✅ <b>Successful:</b> ${successful.length}\n`;
        ...
        if (statusMessage) {
            await bot.editMessageText("❌ An unexpected error occurred while processing the file.", {
                chat_id: chatId, message_id: statusMessage.message_id
            });
        }
    }
